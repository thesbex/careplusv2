...
export function ConsentUploadDialog({ open, onOpenChange, patientId, consultationId }: ConsentUploadDialogProps) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      const annotation = [
        notes.trim(),
        consultationId ? t('consent.upload.attachedNote', { id: consultationId }) : '',
      ]
        .filter(Boolean)
        .join(' · ');
      await upload({
      ...
      toast.success(t('consent.upload.toast.attached'));
      onOpenChange(false);
      setNotes('');
    } catch (err) {
      const msg =
        (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ??
        t('consent.upload.err.failed');
      toast.error(msg);
    } finally {
      setBusy(false);
    }
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <Dialog.Description style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 14, lineHeight: 1.55 }}>
            {t('consent.upload.descPrefix')}<strong>{t('consent.upload.descType')}</strong>
            {consultationId ? t('consent.upload.descLinked') : ''}.
          </Dialog.Description>

          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, marginBottom: 12 }}>
            <span style={{ color: 'var(--ink-3)', fontWeight: 600 }}>{t('consent.upload.noteLabel')}</span>
            <input
              type="text"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder={t('consent.upload.notePlaceholder')}
              aria-label={t('consent.upload.noteAria')}
              style={{
