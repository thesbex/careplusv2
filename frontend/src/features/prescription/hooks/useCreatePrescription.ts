...
function linesToApi(lines: PrescriptionLineDraft[]) {
  return lines
    .filter((l) => l.medication !== null || l.dosage.length > 0 || l.instructions.length > 0)
    .map((l) => ({
      medicationId: l.medication?.id ?? null,
      labTestId: null,
      imagingExamId: null,
      freeText: l.medication ? null : (l.instructions.trim() ? l.instructions : null),
    ...
    mutationFn: async (payload: CreatePrescriptionPayload): Promise<PrescriptionApi> => {
      try {
        const res = await api.post<PrescriptionApi>(
          ...
          {
            type: payload.type,
            lines: linesToApi(payload.lines),
