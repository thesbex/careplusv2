...
export function LetterTemplatesTab() {
  ...
  async function handleSave() {
    ...
    if (!form.body.trim()) {
      toast.error('Le corps du courrier est requis.');
      return;
    }
  ...
  async function handleDelete(t: LetterTemplateView) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Panel style={{ overflow: 'auto', padding: 0 }}>
        ...
        {!isLoading && !error && templates.length === 0 && (
          <div style={{ padding: 24, color: 'var(--ink-3)', fontSize: 13 }}>
            Aucun modèle de courrier. Créez-en un avec le bouton ci-dessus.
          </div>
        )}
        {templates.length > 0 && (
      ...
      {drawerOpen && (
        <div
          role="dialog"
          aria-modal="true"
          aria-label={editingId ? 'Modifier le modèle de courrier' : 'Nouveau modèle de courrier'}
          style={{
          ...
          >
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
              <h2 style={{ fontSize: 14, fontWeight: 650, margin: 0, flex: 1 }}>
                {editingId ? 'Modifier le modèle' : 'Nouveau modèle de courrier'}
              </h2>
              <button
                type="button"
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12, flex: 1, overflow: 'auto' }}>
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12 }}>
                ...
                <input
                  ...
                  placeholder="ex. Demande d'avis cardiologique"
                  style={{
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5 }}>
                ...
                <span style={{ color: 'var(--ink-2)', fontWeight: 600 }}>
                  Actif (proposé au médecin lors de la rédaction)
                </span>
              </label>
            </div>
