...
**Phase**: Post-pilote — **Apparence configurable (thème) + mode sombre livré** (V072, ADR-044) ; écran Chambres & lits refondu « Calm Premium ».
**Last update**: 2026-05-30 (soir) — apparence super admin + dark mode + Chambres & lits Calm Premium
**Build**: frontend `npm run build` vert (tsc strict, exactOptionalPropertyTypes) ; backend `mvn -o compile` vert ; **SuperAdminSettingsIT 10/10** (+3 appearance : round-trip JSON, garde 403 ADMIN normal, taille bornée 400) sur Testcontainers ; Flyway à **v072** (appliqué au boot + en IT, vérifié). Suites FE pré-existantes rouges (UtilisateursTab, StayPrestationsSection, DocumentPdfViewer, routes) — **non liées** (échouent à l'identique sur HEAD sans mes changements).
**QA IHM** : walké au navigateur (Dr Y. El Amrani, SUPER_ADMIN, :5173) — ✅ Chambres & lits « Calm Premium » (cartes sans bordures, statuts en points, numéros accent, tableaux à en-têtes) desktop + mobile 390 ; ✅ panneau **Apparence** (Mode / Canvas / Accent / Police) super admin ; ✅ **mode sombre** appliqué à toute l'app (sidebar, topbar, écrans) ; ✅ persistance cabinet : save → DB `appearance` JSON → reload (cache vidé) recharge depuis le backend ; ✅ reset au défaut. Garde super admin BE (champ protégé comme `language`).
**Next action**: commit (feature + docs + SuperAdminSettingsIT, manuel/IHM d'abord — cf. ship order). Pas de régression connue.

### 2026-05-30 (soir) — Apparence configurable + mode sombre + Chambres & lits « Calm Premium »

Source : maquette Claude Design « careplus refresh - chambres & lits (calm premium).html » (panneau *Tweaks*). Demande : *implémenter Chambres & lits premium + rendre les tweaks configurables par le super admin + mode sombre*.

**Shipped** (pas encore commité) :
- **Thème global configurable** (`lib/theme/appearance.ts` + `AppearanceProvider`) : police · ambiance (canvas, 7 tons dont *stone/calm premium*) · accent · **mode sombre**. Appliqué via variables CSS sur `<html>` (surcharge tokens app v2 + `--ds2-*` du shell). Bootstrap sans flash depuis un cache `localStorage` dans `main.tsx`, reconcilié avec le backend par `AppearanceProvider`.
- **Persistance cabinet, super admin** : colonne `appearance` (JSON) de `configuration_clinic_settings` (**V072**), exposée par `/settings/clinic`, **champ protégé** (garde `requireSuperAdminIfProtectedChanges`, même schéma que `language`/V071). `SettingsController` (JDBC) : GET/UPDATE/INSERT + guard.
- **Panneau Apparence** (`AppearanceSettingsSection`, onglet Cabinet, super admin) : aperçu instantané (preview non persisté) + Enregistrer/Annuler + état *non enregistré*. i18n FR/EN/AR/ES.
- **Chambres & lits « Calm Premium »** : `chambres-lits.css` réécrit (cartes sans bordures + ombre douce, en-tête num+titre hors carte, numéros de section en accent mono, statuts en **points colorés** au lieu de pastilles encadrées, add-form inset, tableaux à en-têtes de colonnes, badge *Inactif* en rouge), 100 % piloté par les tokens → s'adapte à l'ambiance/accent/**sombre**. `ChambresLitsTab.tsx` : ajout des lignes d'en-têtes de colonnes (`cl-rhead`). Wiring backend inchangé.

**Design system « Calm Premium »** (2e fichier Claude Design, `careplus-calm.css`) : c'est exactement le moteur d'apparence ci-dessus piloté par 3 leviers (`data-theme` / `data-canvas` / `--accent`). Réconciliation : la palette sombre + les tons existants correspondent **déjà** aux valeurs canoniques ; complété les **canvas à 10 ambiances** du DS (ajout *clay, cool, lavender, warmgrey* avec les hex exacts) + i18n FR/EN/AR/ES. Vérifié IHM (lavender → `--bg #efedf4`, en RTL arabe). **Non fait (hors scope, assumé)** : re-skin global de tous les écrans avec les primitives `cp-*` (l'écran Chambres & lits les incarne déjà ; à dérouler écran par écran, ADR-021) et redéfinition des couleurs sémantiques médicales (règle couleur DS v2 verrouillée).

**Passe ISO (3e fichier Claude Design, version mise à jour de Chambres & lits + image du panneau Tweaks)** : aligné l'écran et le panneau au pixel.
- **Chambres & lits — drift CSS corrigé** (5 raffinements iso `careplus-calm.css`) : légende = **bloc encadré** (`.cp-legend`), titres de section = **soulignement accent** (`.cp-section-title`), compteur = **pilule bordée**, mini-cartes du tableau des lits = **tuiles blanches bordées + ombre** (`.cp-tile`), boutons fantômes (Désactiver/Supprimer) = **bordés**. Vérifié IHM (RTL arabe).
- **Panneau Apparence complété iso au panneau Tweaks** : ajout des contrôles **Nav active** (encre/accent), **Boutons** (encre/accent), **Logo** (Marque : 6 concepts · Fond · Signe). Câblés bout-en-bout : `--nav-active-bg/fg` (sidebar item actif), `--btn-primary-bg/fg` (`.btn.primary` + `.cl-btn-add`), et `<ConfigurableBrandMark>` (sidebar, réactif à l'aperçu via `subscribeAppearance`/`useSyncExternalStore`). Vérifié IHM live : nav active encre → item noir, boutons encre → CTA noir, logo Care Bloom → marque change. i18n FR/EN/AR/ES.
- **Gap assumé** : navigation mobile `.m-secnav` (stepper de section) non portée — patron de nav de toute la section Paramètres mobile, pas du contenu Chambres ; cf. `docs/FRONTEND_DESIGN_GAPS.md`.

**Passe ISO #3 (design system v3 — catalogue de composants `.cp-*`)** : le DS ajoute un catalogue générique (`.cp-panel/.cp-alert/.cp-av/.cp-badge/.cp-toggle/.cp-check/.cp-seg/.cp-tabs/.cp-search/.cp-tl`). **Tokens de base inchangés** → moteur d'apparence toujours iso. **Décision** : NE PAS porter ces classes `.cp-*` globalement — l'app utilise déjà le préfixe `cp-` pour son shell (`.cp-search`, `.cp-tile`, `.cp-legend`, `.cp-nav-*`…) → collisions + doublons de composants existants. L'app adopte le DS via les **tokens partagés** (le moteur surcharge `--primary/--surface/--ink/…`), mécanisme stated du fichier. **Gap concret fermé** : le panneau 06 « cloisonnement » de Chambres & lits utilisait le `<Panel>` générique + cases natives → porté en variante **calm** (`OrphanRolesPanel calm`) : carte sans bordure + **cases-cartes** (`.cl-ck`, iso `.cp-check`) + hint « Enregistré automatiquement ». Opt-in (prop `calm`) → vaccination/grossesse inchangés. Vérifié IHM (isolation activée le temps du test puis remise à OFF). L'écran Chambres & lits est désormais **iso sur ses 6 panneaux**.

**Passe ISO #2 (design system v2 — `careplus-calm.css` enrichi)** : le DS ajoute 2 primitives vs la version précédente — `.cp-ghost` (boutons fantômes bordés, **déjà iso** dans Chambres) et un **menu déroulant custom** (`.dd`, remplace le popup natif `<select>`). Porté : les 4 `<select>` du formulaire Chambres & lits (Service · Classe · Chambre · règle de facturation) basculés sur le composant app `SelectMenu` (popup stylé porté, navigation clavier, coche de sélection) — équivalent idiomatique de `.dd` — calés sur les champs Calm Premium via `.cl-selm`. Vérifié IHM : popup stylé (pas de liste OS), sélection commit, plus aucun `<select>` natif dans les add-forms. La pastille de statut de lit (`.cl-statsel`) reste un contrôle dédié à points colorés (hors primitive `.dd`).

**Décisions** : ADR-044 (apparence/thème). Tweaks *logo / nav-style / button-style* de la maquette **écartés** (le shell de l'app n'a pas d'équivalent « nav fill ink » et le logo passe déjà par `LogoSettingsSection`) — sous-ensemble app-wide retenu : police, ambiance, accent, sombre.

**State** : FE `npm run build` 🟢 (tsc strict) · BE `mvn -o compile` 🟢 · **SuperAdminSettingsIT 10/10** (+3 appearance) sur Testcontainers · V072 appliquée au boot + en IT (vérifié log Flyway). QA IHM verte desktop + mobile, clair + sombre, save/persistance prouvée en base. Échecs FE pré-existants non liés (confirmé : identiques sur HEAD sans mes changements).
**Next** : commit (feature + docs + IT).
