...
| 6 | Paramétrage + Onboarding | ✅ DONE (Paramétrage) — Onboarding ⏳ post-MVP | _next push_ | Backend : nouveau `SettingsController` (`GET/PUT /api/settings/clinic`, `GET /api/settings/tiers`, `PUT /api/settings/tiers/:tier`) — JdbcTemplate sur `configuration_clinic_settings` + `config_patient_tier`. Frontend : `ParametragePage` 4 onglets (Cabinet, Tarifs, Utilisateurs, Congés). Hooks `useClinicSettings` / `useUpdateClinicSettings` / `useTiers` / `useUpdateTierDiscount` / `useUsers` / `useCreateUser` / `useDeactivateUser`. Onboarding (écran 13 prototype) reste statique — on peut bootstrap via `/api/admin/bootstrap` directement. |
| 7 | Queue enrichi (`age`, `reasonLabel`, `practitionerName`) | ✅ DONE | _next push_ | Backend : `QueueEntryView` + SQL JOIN exposent `age` (calculé via Period), `reasonLabel`, `practitionerName` (Dr. xxx), `durationMinutes`, `isPremium`. Frontend : `QueueRow` rend les vraies valeurs au lieu de `0 ans` / `—`, étoile 🌟 sur Premium, durée slot affichée. Sub date dynamique fr-MA. |
| 8 | Playwright E2E + Postman + tag `v0.1.0-mvp` | ✅ DONE | _next push_ | `playwright.config.ts` + 3 specs (RDV check-in / consultation+sign+PDF / invoice issue+pay+PAYEE_TOTALE). Vitest exclude `e2e/**`. `docs/API.md` à jour avec settings + catalogue référentiels + queue enrichi. Postman : importer directement `http://localhost:8080/v3/api-docs`. Tag `v0.1.0-mvp` posé. |
