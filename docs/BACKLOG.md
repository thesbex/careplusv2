...
- [x] **QA9-1 — Photo de profil des collègues dans les contacts de chat** — **[BUG]** · ✅ **LIVRÉ 2026-05-26**. *« La photo d'un collègue n'apparaît pas dans les contacts de chat, il faut mettre sa photo de profil. »* Infra déjà en place (V052 `identity_user.photo_*`, `GET /api/users/{id}/photo`, upload via Profil `PUT /api/me/photo`, DTO chat exposant `hasPhoto`+`id`, composant `UserAvatar`). `UserAvatar` était déjà câblé dans la liste DM, le panneau membres et le picker — **manquait l'en-tête de conversation** (pile d'avatars `members.slice(0,4)` en initiales dur) **et le picker @mention**. Branchés sur `UserAvatar` (+ prop `style` ajoutée pour la pastille ronde superposée). Walk IHM : upload photo via Profil → photo affichée dans l'en-tête de DM (et liste/membres/mention). IT : `UserAvatar.test.tsx` (initiales sans photo, fusion style rond, `<img>` via /users/{id}/photo, fallback initiales sur 404).
...
- [x] **QA9-3 — Photo de profil dans la liste "Nouveau message"** — **[FEATURE]** · ✅ **DÉJÀ LIVRÉ — vérifié 2026-05-26**. *« Dans l'écran de nouveau message où on choisit un interlocuteur, rajoute l'image de profil des utilisateurs dans chaque élément de la liste du personnel. »* Le `ColleaguePicker` (« Nouveau message ») utilise déjà `UserAvatar` (`hasPhoto`+`id` exposés par `GET /chat/colleagues`). Vérifié IHM avec la photo uploadée. Couvert par le même composant que QA9-1.
- [x] **QA9-4 — Icône "patient" inerte dans la barre du chat → activer + fonctionnalité** — **[FEATURE]** · ✅ **DÉJÀ LIVRÉ — vérifié 2026-05-26** (activé par un lot R0xx antérieur). *« Dans l'écran de chat, à droite du bouton de chargement de documents, une icône avec le libellé "patient". À quoi sert-elle ? Je veux que tu l'actives et offres une fonctionnalité derrière. »* Fonctionnalité = **joindre une fiche patient au message** : le bouton « Patient » ouvre un picker → chip « Patient joint : X » → à l'envoi, le destinataire voit une carte « Nom · PID · âge · Ouvrir dossier » sous la bulle. Backend : `SendMessageBody.patientId` + `MessageView.AttachedPatient`. Walk IHM : joindre Ahmed Cherkaoui → message envoyé → carte + bouton « Ouvrir dossier » rendus, **persistés après reload**. IT : `ChatIT.message_with_attached_patient_echoes_patient_card` (POST avec patientId → `$.patient.id`/`name` + persistance GET).

### Pharmacie interne / médicaments fournis par l'établissement (3 — bundle cohérent)

- [x] **QA9-5 — Capability "l'établissement fournit des médicaments en interne"** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. Flag `pharmacy_internal` (V057) + toggle « Pharmacie interne » dans Paramétrage > Services internes (calqué sur lab/imaging interne). Walk IHM : activé → persiste après reload.
- [x] **QA9-6 — Prix des médicaments fournis (comme radiologie/imagerie)** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. `catalog_medication.internal_price` (V057, calqué sur `catalog_lab_test.internal_price` / `catalog_imaging_exam.internal_price` de V050) + champ « Prix de cession en interne (MAD) » dans le drawer Catalogue > Médicaments, visible seulement si pharmacie interne activée. Walk IHM : prix 50 sur Doliprane 1g → persiste après reload.
- [x] **QA9-7 — Prescription "fournie en interne" → mise à jour automatique de la facture** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. Toggle « Fournir en interne (pharmacie) » dans le `PrescriptionDrawer` DRUG (visible si pharmacie interne activée) → `clinical_prescription_line.internal_dispense=true` (V057). À la signature, `BillingService.onConsultationSigned` ajoute une ligne facture `internal_price × quantité` par médicament fourni en interne (skip si prix NULL). **Walk IHM bout-en-bout** : Doliprane 1g (prix 50) × 2 fourni en interne → facture = Consultation 200 + « Médicament (interne) : Doliprane 1g » 50×2=100 → total 300. IT : `BillingIT` (2 scénarios : 45×2=90/total 440 ; prix NULL non facturé) + vitest `linesToApi` (propagation flag DRUG — **bug attrapé en QA IHM** : le mapping front forçait `internal=false` sur DRUG). **À cadrer plus tard** : décrément stock interne (lien QA7-1). Pas de décrément stock en v1.
...
- [x] **QA9-10 — Notes professionnelles / courriers à un confrère externe, associées à la consultation** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. Module `ma.careplus.confrere` : depuis la consultation, action « Courrier confrère » (desktop + mobile) → dialog (destinataire prérempli depuis l'annuaire « Mes confrères » V046 ou saisie libre + corps libre) → `POST /consultations/{id}/confrere-letter` rend un PDF (en-tête/logo/filigrane + signature comme l'ordonnance) stocké en `patient_document` type `LETTRE_CONFRERE`, rattaché à la consultation (notes `consultationId=…`), téléchargé immédiatement. `GET /consultations/{id}/confrere-letters` liste les courriers. IT `ConfrereLetterIT` (6) + vitest `ConfrereLetterDialog` (4). Concrétise F13. Build vert.

### Liste d'attente multi-praticien (2)

- [x] **QA9-11 — Salle d'attente en colonnes par médecin (secrétaire multi-praticien) + cloisonnement** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. `QueueEntryView`/`PresenceService` exposent désormais `practitionerId` (la file est déjà filtrée serveur par cloisonnement). Salle d'attente : quand ≥2 praticiens actifs → colonnes par médecin (desktop, une `Panel` par médecin + « Non affecté ») ; mobile → chips filtre par médecin. Mono-praticien / MEDECIN cloisonné → liste plate inchangée. IT existants + vitest colonnes. *« Si une secrétaire gère plusieurs médecins, la liste d'attente devrait être divisée en colonnes pour séparer les visites par médecin. Un médecin ne voit que ses consultations (si cloisonnement activé). »* S'appuie sur la direction multi-praticien (cf. [[multi-practitioner-direction]], section *Multi-practitioner cabinet* du backlog) et le cloisonnement (cf. [[cloisonnement-grossesse-v039]] / ADR-032). UI : colonnes Kanban par praticien quand ≥2 MEDECIN actifs ; vue mono filtrée pour un médecin cloisonné.
- [x] **QA9-12 — Ajout patient spontané (sans RDV) en salle d'attente** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. Bouton « Ajouter un patient sans RDV » (desktop + mobile) → `AddWalkInDialog` (recherche patient + choix praticien — présélection self si MEDECIN / unique praticien) → crée un RDV `walkIn:true, urgency:true, startAt=now` puis check-in → le patient apparaît dans la colonne du médecin choisi. Backend déjà suffisant (POST /appointments + check-in). vitest walk-in. *(Drag-drop inter-colonnes non requis : l'affectation se fait au moment de l'ajout via le sélecteur praticien.)* *« Un patient peut se présenter spontanément sans rendez-vous ; la secrétaire doit pouvoir le rajouter à l'écran de salle d'attente (en le glissant directement dans la bonne colonne si elle gère plusieurs médecins). »* Dépend de QA9-11 (les colonnes). Bouton "Ajouter un patient sans RDV" → recherche/création patient → place dans la file du médecin choisi (drag-drop entre colonnes).

### Administration / RH / charges (3)

- [x] **QA9-13 — Modèles de consentement par type, créés par l'admin, réutilisés par les médecins** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. Module `ma.careplus.consent` (V059 `clinical_consent_template`) : 7 types (PARTAGE_DOSSIER, ACTE_OPERATOIRE, ANESTHESIE, IMAGERIE, PRELEVEMENT, HOSPITALISATION, AUTRE), titre + corps avec placeholders `{{patientNom}}/{{patientCin}}/{{dateJour}}/{{cabinet}}`. Onglet « Consentements » dans Paramétrage (ADMIN, CRUD). Côté dossier patient (desktop onglet Documents + mobile onglet Admin.) : bouton « Générer un consentement » → choix d'un modèle → édition titre/corps → `POST /patients/{id}/consents` rend un PDF (en-tête/logo/filigrane comme l'ordonnance) stocké en `patient_document` type `CONSENTEMENT`, téléchargé immédiatement. IT `ConsentTemplateIT` (11) + vitest `ConsentTemplatesTab`. Build vert.
- [x] **QA9-14 — Module personnel/RH : utilisateurs sans accès appli + salaire + congés auto** — **[FEATURE]** · ✅ **v1 LIVRÉ 2026-05-26** *(« HADI WA7CH number 2 »)*. Module `ma.careplus.hr` (V061) : `hr_staff` (poste SECURITE/MENAGE/INFIRMIER/SECRETAIRE/ASSISTANTE/TECHNICIEN/AUTRE, date recrutement, salaire mensuel, téléphone, `user_id` optionnel pour assistante/secrétaire avec login), `hr_leave_entry` (CONGE/ABSENCE/RETARD), `hr_salary_payment`. Endpoints ADMIN `/api/hr/staff` + `/leave` + `/payments` + `/summary` (**accrual congés AUTO = 1,5 j × mois travaillés**, solde = acquis − congés pris, compteurs absences/retards). Écran `/personnel` (desktop + mobile, ADMIN, câblé Sidebar + menu mobile) : liste staff + drawer détail (solde congés calculé, saisie congés/absences/retards, log paiements salaire). IT `HrStaffIT` (11) + vitest. **v2 différé** : bulletin de paie PDF, workflow d'approbation congés, stats retards. *Couvre les demandes explicites : staff sans accès appli, salaire, date recrutement, accrual 1,5j/mois auto, gestion salaire+congés, assistante/secrétaire inclus.* *« Créer des utilisateurs sans accès à l'application pour suivre paiement, congés, absences, retards (agent de sécurité, femme de ménage, infirmière…). L'admin définit le salaire, la date de recrutement ; le système met à jour automatiquement le solde de congé de 1,5 jour par mois travaillé ; administrer salaire et congés. Les profils assistante/secrétaire sont aussi concernés. »* Périmètre à cadrer (brainstorming requis) : entité `staff_member` (login optionnel/désactivé, poste, date recrutement, salaire), accrual congés 1,5 j/mois automatique (job mensuel), suivi absences/retards, fiche de paie simple. **À trancher** : lien avec `identity_user` existant (assistante/secrétaire = déjà des users) vs entité RH séparée. *Suggestions bienvenues de l'utilisateur.*
- [x] **QA9-15 — Écran de gestion des charges (eau/électricité, internet, loyer, syndic, réparations…)** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. Module `ma.careplus.finance` (V058 `finance_expense`) : catégorie (10 valeurs contrôlées), libellé, montant, date, périodicité PONCTUELLE/MENSUELLE/ANNUELLE, fournisseur, notes, soft-delete. Endpoints `GET/POST/PUT/DELETE /api/expenses` (lecture MEDECIN+ADMIN, écriture ADMIN) + `GET /api/expenses/summary?year=` (totaux mensuels). Écran `/charges` (desktop + mobile) gated ADMIN, câblé Sidebar **et** menu mobile : table CRUD + drawer + récap annuel (total + 12 mois). IT `FinanceExpenseIT` (8) + vitest `ChargesPage`. Walk IHM : charge « Loyer cabinet mai » 4000 MAD créée → apparaît dans la table + Total 2026 = 4000 MAD ; mobile rend la page. *Justificatif PDF + lien dashboard financier (F1, CA−charges) différés.*
...
- [x] **QA10-1 — Compte-rendu de séjour : config logo (filigrane/background) non appliquée** — **[BUG]** · ✅ **CORRIGÉ 2026-05-26**. *« …le logo en background n'est pas appliqué sur le document généré. »* Le template `hospitalisation-cr.html` avait bien le bloc filigrane, mais `StaySummaryPdfService` passait le logo brut : or **openhtmltopdf ignore l'opacité CSS sur les images raster**, donc le filigrane ne s'appliquait pas (logo pleine opacité / absent). `PrescriptionPdfService` contournait déjà ça en cuisant l'alpha dans le PNG côté serveur (`LogoWatermarkRenderer.applyTransparency`). Fix : `StaySummaryPdfService` applique le même traitement quand `logo_position=WATERMARK`. IT : `StayExtrasIT.x3b_pdfWithWatermarkLogo` (logo PNG WATERMARK → CR PDF valide `%PDF`). Confirmation visuelle recommandée sur env déployé (mécanisme désormais identique au filigrane ordonnance qui fonctionne).
- [x] **QA10-2 — Prestations pendant l'hospitalisation (consultation, O₂, repas…) → association patient + facturation** — **[FEATURE]** · ✅ **LIVRÉ 2026-05-26**. V060 `hospitalization_stay_prestation` (stay_id, act_id?, label, unit_price, quantity, performed_at). `POST/GET/DELETE /api/hospitalization/stays/{id}/prestations` (DELETE refusé 409 après facturation). `generateInvoice()` ajoute une ligne facture par prestation → **total séjour = journées + prestations**. `StayDetailView` expose `prestations` + `prestationsTotal`. Frontend : section « Prestations » dans `StayDetailPanel` (desktop + mobile) — dropdown catalogue (préremplit libellé+prix) ou saisie libre + qté, liste + total, suppression, aperçu facturation rafraîchi. IT `StayPrestationIT` (7, ex. Oxygène 120×2 → +240) + vitest (5). Build vert.
- [x] **QA10-3 — Accès à l'historique d'hospitalisation après la sortie** — **[FEATURE]** · ✅ **ADRESSÉ 2026-05-26**. *« Une fois la personne n'est plus hospitalisée, il n'y a pas moyen de revenir à l'historique. »* L'onglet « Séjours » du dossier patient (desktop + mobile, livré ce jour — commit `7c4bd93`) liste **tous** les séjours du patient quel que soit le statut (EN_COURS / SORTI / FACTURÉ / ANNULÉ) via `GET /hospitalization/stays?patientId=` → `StaysTab` affiche dates, statut coloré, motif, lit, coût. L'historique post-sortie est donc accessible depuis le dossier. Reste optionnel : un filtre « sortis » / archive dans la worklist `/hospitalisation` (différé, non bloquant).
- [x] **QA10-4 — « Générer facture de séjour » : toast succès mais facture absente de Facturation** — **[BUG]** · ✅ **CORRIGÉ 2026-05-26**. *« …message "Facture de séjour générée. Ouverture dans Facturation" mais dans la facturation je ne retrouve pas la facture. »* Cause : le bouton navigue vers `/facturation?invoice=<id>` mais `FacturationPage` **ignorait** le paramètre `invoice` ; or la facture séjour est en BROUILLON et tombe en bas de la liste (tri émission DESC NULLS LAST), donc l'utilisateur ne la « retrouvait » pas. Fix : `FacturationPage` lit `?invoice=<id>` au montage et ouvre directement le tiroir de cette facture (deep-link), conforme au toast « Ouverture dans Facturation ». Walk IHM : `/facturation?invoice=<id>` ouvre le tiroir (patient + total corrects). IT : `FacturationPage.filters.test.tsx` (le tiroir s'ouvre sur la facture du paramètre). NB : la facture séjour (`consultation_id` NULL) reste filtrée par un éventuel filtre médecin — tracé pour un durcissement ultérieur de `InvoiceSpecifications` si besoin.
